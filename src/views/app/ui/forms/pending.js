...
import { sortTable } from 'helpers/Utils';
...
const Pending = ({ match }) => {
  ...
  const [sortedBy, setSortedBy] = useState(''); // all about sorting
    ...
    async function fetchData() {
      database.ref('/purchases').on('value', (snapshot) => {
        ...
        Object.keys(dataRes).forEach(function eachKey(key) {
          ...
          // if (dataRes[key].info?.type === 'family') {
    ...
    if (e.target.value !== '') {
      const filteredData = familyGroup.filter((elem) => {
        return (
          ...
          // elem.info.type.toLowerCase().includes(e.target.value.toLowerCase())
        );
      });
      setSearchValue(filteredData);
    } else {
  ...
  const onDownload = () => {
    if (familyGroup.length > 0) {
      ...
      console.log(dataToDownload, 'pending');
      setTimeout(() => {
        csvLink.link.click();
      }, 3000);
    }
  ...
  const markAsComplete = (id) => {
    ...
    database.ref(`/purchases/${id}`).update({ sent: true })
  }


  // Sorting Table by util function
  ...
  return (
    <>
      ...
      <Card className="mb-4">
        <CardBody>
          <CardTitle>Pending Magazines</CardTitle>
          ...
          <Table responsive>
            <thead>
              <tr>
                <th>#</th>
                <th>Group ID</th>
                <th style={{ cursor: 'pointer' }} onClick={(e) => handleSortTable('groupName', e)}>Group Name</th>
            ...
            <h1>The data is here from database</h1>
            <tbody>
              {
                console.log(searchValue[3], ' pending')
              }
                  ...
                  <tr key={index}>
                    ...
                    <td>
                      <Button
                        ...
                        className="simple-icon-check"
                        style={{ fontSize: '25px' }}
                        onClick={() => markAsComplete(item.key)}
                      // target="_blank"
                      // href={item.magazine.pdfURL}
                      // disabled={isNil(item.magazine.pdfURL)}
                      ></Button>
