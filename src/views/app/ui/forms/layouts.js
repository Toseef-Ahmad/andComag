...
const FormLayoutsUi = ({ match }) => {
  ...
  useEffect(() => {
    async function fetchData() {
      database.ref('/purchases').orderByChild('sent').equalTo(true).on('value', (snapshot) => {
  ...
  const onGenerate = (magazineKey) => {
    database
      .ref(`/magazines/${magazineKey}/generate`)
      .on('value', (snapshot) => {
        const dataRes = snapshot.val();
        console.log(dataRes, 'this is hhhhhhhh');
      });
  };
  ...
  return (
    <>
      ...
      <Card className="mb-4">
        <CardBody>
          <CardTitle>Complete Magazines</CardTitle>
                  ...
                  <tr key={index}>
                    ...
                    <td>
                      <Button
                        ...
                        style={{ fontSize: '25px' }}
                        onClick={() => onGenerate(item.magazine.key)}
                      // target="_blank"
                      // href={item.magazine.pdfURL}
                      // disabled={isNil(item.magazine.pdfURL)}
                      ></Button>
